...
function TitleScreen({
  ...
  const ducking = exitPhase === 'duck' || exitPhase === 'reveal' || !monsterRisen
  const isMobileTitle = window.innerWidth < 768
  const offscreenY = isMobileTitle ? '128%' : '118%'
  const monsterPose = ducking
    ? `translateX(-50%) translateY(${offscreenY})`
    : isNear
  ...
  const monsterWrap = (z) => ({
    ...
    transform: monsterPose,
    transition: 'transform 0.42s cubic-bezier(.34,1,.5,1)',
    pointerEvents: 'none', zIndex: z,
  })
  const breathStyle = { animation: ducking ? 'none' : 'tomaBreath 3.5s ease-in-out infinite', transformOrigin: 'center bottom' }
  ...
  return (
    <div className="fill-viewport" style={{
      ...
      {/* z:1 — body + hat rise together (hat glued to head, no separate drop) */}
      <div style={monsterWrap(1)}>
        <div style={{ ...breathStyle, position: 'relative' }}>
      ...
      {/* z:3 — face / eyes (in front of title text), shares pose + breath with body */}
      <div ref={faceRef} style={monsterWrap(3)}>
